...
int gtc_mem_free(gtc_bench_data_t* gtc_input) {
    ...
    particle_point = &(gtc_input->particle_point);
    ...
    _mm_free(field_data->adum);
    _mm_free(field_data->adum2);
...
#if FOURPOINT
    _mm_free(particle_point->point_index_count);
#endif
...
int setup(gtc_bench_data_t* gtc_input) {
    ...
    radiald_comm = &(parallel_decomp->radiald_comm);
    radial_partd_comm = &(parallel_decomp->radial_partd_comm);  
...
#pragma omp parallel for
    ...
    for (i=0; i<mpsi+1; i++){
      ...
      if (rad - a_nover_in > -1.0e-12 && a_valid_in == -1.0) {
        a_valid_in = rad;
      ...
      if (a_valid_out == -1.0 && rad - a_nover_out > -1.0e-12){
        a_valid_out = rad;
    ...
    if (myrank_radiald==nproc_radiald-1&& ipsi_out != mpsi){
      printf("mype=%d ipsi_out %d /= mpsi %d rad=%e a_nover_in=%e a_nover_out=%e a_valid_out=%e\n ", mype, ipsi_out, mpsi, rad, a_nover_in, a_nover_out, a_valid_out);
...
#if FOURPOINT
    point_index_count = (my_int *) _mm_malloc(nloc_over*sizeof(my_int), IDEAL_ALIGNMENT);
#endif
...
#pragma omp parallel for private(i)
    for (i=igrid_nover_in_radiald - igrid_in; i<igrid_nover_out_radiald - igrid_out +1; i++){
        perr[i]      = 0.0;                                                                             
        ptilde[i]    = 0.0;                                                                             
        phitmp[i]    = 0.0;                                                                             
        dentmp[i]    = 0.0;    
...
#pragma omp parallel for private(i)
    ...
    for (i=ipsi_in; i<ipsi_out+1; i++){
      r = a0 + deltar*i;
      rmax = gtc_min(a1,r+0.5*deltar);
      rmin = gtc_max(a0,r-0.5*deltar);
    ...
    for (k=1; k<mzeta+1; k++) {
        ...
        for (i=ipsi_in; i<ipsi_out+1; i++) {
            for (ip=1; ip<3; ip++) {
                indp = gtc_min(ipsi_out, i+ip);
                indt = gtc_max(ipsi_in, i-ip);
                for (j=0; j<mtheta[i]+1; j++) {
                    ij = igrid[i] + j;
    ...
    scalar_data = (real *) _mm_malloc(16*sizeof(real), IDEAL_ALIGNMENT);
...
#pragma omp parallel for private(i)
    ...
    */
    ...
    for (m=0; m<mi; m++) {
      ...
      assert(z0[m]>=a_nover_in);
      assert(z0[m]<=a_nover_out);
    }
    ...
    for (m=0; m<mi; m++) {
        ...
        z3[m]  = sqrt(gtc_max(1e-20,log(1.0/gtc_max(1e-20, pow(z3[m],2)))));
    ...
    for (m=0; m<mi; m++) {
        ...
        z3[m] = z03[m]*gtc_min(umax,z3[m]);
        z4[m] =
            2.0*w_initial*(z4[m]-0.5)*(1.0+cos(z1[m]));
#if NORMAL_DIST
        zt1 = gtc_max(1.0e-20,z5[m]);
        /* (z5[m] > 1.0e-20) ? z5[m] : 1.0e-20; */
        zt2 = gtc_min(2.0*umax*umax,-2.0*log(zt1));
        /* (umax*umax < -log(zt1)) ? umax*umax : -log(zt1); */
        z5[m] = gtc_max(2.0e-20,zt2);
#else
    ...
    for (m=0; m<mi; m++) {
      ...
      z05[m] = velocity_vol*pow(2.0*pi, -1.5)*exp(-energy*tem);     
      ...
      z05[m] = velocity_vol*pow(2.0*pi,-1.5)*exp(-kappa*tanh(rfac1)/rw)*exp(-kappa2*(energy * tem));
#if FLAT_PROFILE
      z05[m] = velocity_vol*pow(2.0*pi, -1.5)*exp(-energy*tem);
#endif
#endif
...
#pragma omp parallel for private(i)
    ...
    if (mype==0&&global_params->tauii>0.0){
    ...
    field_data->adum = adum; field_data->adum2 = adum2;
...
#if FOURPOINT
    particle_point->point_index_count = point_index_count;
#endif
