...
#pragma omp for
	...
	continue;
      ...
      ip = gtc_max(0,gtc_min(neop-1, (int)((0.5*r*r-psimin)*delp)));
      // use cell in theta for near equal volume   
      jt = gtc_max(0,gtc_min(neot-1, (int)(zion1m*delt)));
      // use cell in global zeta address
      kz = gtc_max(0,gtc_min(neoz-1, (int)(zion2m*delz)));
      ...
      v = sqrt(gtc_max(vmin, upara*upara+uperp2));
      zv = gtc_max(0.1, gtc_min(10.0, v*vths));
      xv = zv*zv;
      
      freq = 1.88*(real)ndiag*tstep/(tauii*zv*xv);

      // maxwellian integral by table look-up for range of (0.0001, 10.0)
      k = gtc_min(100000, (int)(xv*10000.0)+1);
      ...
      dp = gtc_max(v3, upara*upara*h + uperp2*g);
      dn = gtc_max(v5, 4.0*uperp2*v*v*v*v*g*h/dp);
	...
	delv2 = 0.0;
      ...
      upara = upara + delu;
      uperp2 = gtc_max(vmin, uperp2 + delv2);
...
#pragma omp parallel for
  for (int m=0; m<mi; m++){
    ...
    real v = sqrt(gtc_max(vmin, zion00m*zion00m+zion01m));
    real zv = gtc_max(0.1, gtc_min(10.0, v*vths));
    real xv = zv*zv;

    // maxwellian integral by table look-up for range of (0.0001, 10.0)
    int k = gtc_min(100000, (int)(xv*10000.0)+1);
