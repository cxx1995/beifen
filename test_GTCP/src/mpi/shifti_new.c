    ...
    while (iteration <= ntoroidal) {
{
	...
	int rrequestleft, rrequestright;
...
#pragma omp barrier
	...
	start_t = MPI_Wtime();
        ...
        /* allocate mem for sending particle data */
	...
	MPI_Irecv(recvleft, mrecvleft*nparam, MPI_MYREAL, parallel_decomp->left_pe, 
		  parallel_decomp->left_pe, parallel_decomp->toroidal_comm, &rrequestleft);

	MPI_Irecv(recvright, mrecvright*nparam, MPI_MYREAL, parallel_decomp->right_pe, 
		  parallel_decomp->right_pe, parallel_decomp->toroidal_comm, &rrequestright);
...
#pragma omp parallel for private(pos)
        ...
        MPI_Send(sendright, msendright*nparam, MPI_MYREAL, parallel_decomp->right_pe,
                 parallel_decomp->myrank_toroidal, parallel_decomp->toroidal_comm);
...
#pragma omp parallel for private(pos)
	...
	MPI_Send(sendleft, msendleft*nparam, MPI_MYREAL, parallel_decomp->left_pe, 
		 parallel_decomp->myrank_toroidal, parallel_decomp->toroidal_comm);

	MPI_Wait(&rrequestleft, &istatus1);
...
#pragma omp parallel for private(pos)
	...
	MPI_Wait(&rrequestright, &istatus2);
...
#pragma omp barrier
	...
	printf("radial mype=%d msendleft=%d msendright=%d\n", parallel_decomp->mype, msendleft, msendright);           
      */
      ...
      MPI_Irecv(recvleft, mrecvleft*nparam, MPI_MYREAL, radial_decomp->left_radial_pe,
		radial_decomp->left_radial_pe, parallel_decomp->partd_comm, &rrequestleft);

      MPI_Irecv(recvright, mrecvright*nparam, MPI_MYREAL, radial_decomp->right_radial_pe,
		radial_decomp->right_radial_pe, parallel_decomp->partd_comm, &rrequestright);
...
#pragma omp parallel for private(pos)
	...
	sendright[nparam*i+12] = z06[pos];
      ...
      /* send particles to right neighbor */
      MPI_Send(sendright, msendright*nparam, MPI_MYREAL, radial_decomp->right_radial_pe,
	       parallel_decomp->myrank_partd, parallel_decomp->partd_comm);
...
#pragma omp parallel for private(pos)
	...
	sendleft[nparam*i+12] = z06[pos];
      ...
      /* send particles to left neighbor */
      MPI_Send(sendleft, msendleft*nparam, MPI_MYREAL, radial_decomp->left_radial_pe,
	       parallel_decomp->myrank_partd, parallel_decomp->partd_comm);
