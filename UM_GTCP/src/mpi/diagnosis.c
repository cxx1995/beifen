...
int diagnosis(gtc_bench_data_t *gtc_input) {
  ...
  real *fdum = (real *) _mm_malloc((12+2*mflux)*sizeof(real), IDEAL_ALIGNMENT);
  real *adum = (real *) _mm_malloc((12+2*mflux)*sizeof(real), IDEAL_ALIGNMENT);
  assert(xnormal!=NULL);
  assert(fdum!=NULL);
  assert(adum!=NULL);
  //real fdum[21];
  ...
  */
  ...
  for (int i=0; i<mflux; i++){
    fdum[12+i] = eflux[i]*tem_inv/vthi;
    fdum[12+mflux+i] = rmarker[i];
  }

#if USE_MPI
  MPI_Reduce(fdum, adum, 12+2*mflux, MPI_MYREAL, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
  ...
  if (mype==0){
    ...
    real tmarker= 0.0;
    for (int i=0; i<mflux; i++){
      fdum[12+mflux+i] = gtc_max(1.0, fdum[12+mflux+i]);
      fdum[12+i] = fdum[12+i]*xnormal[i]/fdum[12+mflux+i];
      tmarker = tmarker + fdum[12+mflux+i];
    }
    ...
    printf("istep+mstepall=%d efield=%e eradial=%e entropyi=%e, dflowi=%e, pfluxi=%e, efluxi=%e eflux[2]=%e rmarker[2]=%e particles_energy[0]=%e particles_energy[1]=%e, sum_of_weights=%e\n",
           istep+mstepall, fdum[0], eradial, fdum[1], fdum[3], fdum[5], fdum[7], fdum[12+mflux/2], fdum[12+mflux+mflux/2], fdum[9], fdum[10], fdum[11]);
    ...
    fprintf(pFile, "%d %e %e %e %e %e %e %e %e %e %e %e\n",  istep+mstepall, fdum[0], eradial, fdum[1], fdum[3], fdum[5], fdum[7], fdum[12+mflux/2], fdum[12+mflux+mflux/2], fdum[9], fdum[10], fdum[11]);
    fclose(pFile);
