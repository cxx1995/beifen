...
{
//printf("call_gpu_charge_kernel********** line 746\n");
...
//modified by cxx 2017/5/7 20:21
...
//printf("d_grid->densityi: %x line 756 \n",&d_grid->densityi);
//printf("h_grid->densityi: %x line 757 \n",&h_grid->densityi);

//cudaMallocManaged(&d_grid->densityi,(mzeta+1)*nloc_over* sizeof(wreal));
//printf("d_grid->densityi: %x line 758 \n",&d_grid->densityi);
d_grid = &(gpu_kernel_input->d_grid);

  gtc_field_data_t *h_grid = &(gtc_input->field_data);
//  gtc_radial_decomp_t *h_radial_decomp = &(gtc_input->radial_decomp);
//printf("d_grid->densityi: %x line 765 \n",&d_grid->densityi);
//printf("h_grid->densityi: %x line 766 \n",&h_grid->densityi);    
...
//printf("d_grid->densityi: %x line 803 \n",&d_grid->densityi);
//printf("h_grid->densityi: %x line 804 \n",&h_grid->densityi);  
...
//	CUDA_SAFE_CALL(cudaMemcpy(h_aux_zion->jtion1, (void *)d_aux_zion->jtion1, 4*(mi)*sizeof(int) , cudaMemcpyDeviceToHost));
	...
	for(m=0;m<mi;m++)
	  fprintf(fd,"%d %d\n", d_aux_zion->jtion0[m],d_aux_zion->jtion1[m]);
	fclose(fd);
	irk2_save = 1;
      }
    } else {
