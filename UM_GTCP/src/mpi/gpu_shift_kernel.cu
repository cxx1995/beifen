...
//modified by cxx 2017/5/8 13:00
__device__ __managed__ unsigned int shift_flush_top[SHIFT_COUNT] __align__ (16);
...
memcpy(shift_flush_top, tops, 3 * sizeof(unsigned int));
...
memcpy(tops, shift_flush_top, sizeof(unsigned int) * 3);
...
memcpy(sends[1], gpu_kernel_input->ptr_d_zion_shift, NPARAMS * tops[1] * sizeof(real));
...
memcpy(sends[2], (gpu_kernel_input->ptr_d_zion_shift + size*NPARAMS), NPARAMS * tops[2] * sizeof(real));
...
memcpy(gpu_kernel_input->ptr_d_zion_shift,particle_data, NPARAMS *mi_append * sizeof(real));
	gpu_kernel_input->gpu_timing.memtransfer_shift_time +=  gpu_timer_measure(gpu_kernel_input);
