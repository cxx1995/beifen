...
__device__ __managed__ int d_start_shift[3];//modified by cxx 2017/5/4 22:50
...
{
  //   printf(" call_gpu_bin_particles_kernel***********\n");
...
//printf("d_start_shift[0]:%x line 446\n",d_start_shift[0]); 
...
//modified by cxx 2017/5/5 h_start_shift to d_start_shift
extern "C"
...
{
//     printf("d_start_shift[0]:%x  line 457\n",d_start_shift[0]);
//     printf("call_gpu_shifti_extract_kernel****************\n");
     gpu_timer_start(gpu_kernel_input);
    
//modified by cxx 2017/5/4 22：23
//int *temp_start_shift;
//CUDA_SAFE_CALL(cudaMallocManaged(&temp_start_shift, sizeof(int)*3)); 
//CUDA_SAFE_CALL(cudaMemcpyFromSymbol(temp_start_shift, d_start_shift, sizeof(int)*3,0,cudaMemcpyDeviceToDevice));

//memcpy(temp_start_shift,d_start_shift,sizeof(d_start_shift));
//     CUDA_SAFE_CALL(cudaMemcpyFromSymbol(h_start_shift, d_start_shift, sizeof(int)*3,0,cudaMemcpyDeviceToHost));
     ...
     int mi = gtc_input->global_params.mi;
//modified by cxx 2017/5/4 22：23   h_start_shift to temp_start_shift  
//  printf("mi:%x line 472\n",mi); 
//printf("h_start_shift[0]:%x\n",h_start_shift[0]); 
//printf("*temp_start_shift:%s\n",*temp_start_shift);
//printf("temp_start_shift[0]:%s\n",temp_start_shift[0]);
//printf("d_start_shift[0]:%x  line 476\n",d_start_shift[0]);
assert(mi == d_start_shift[0]);
//printf("d_start_shift[0]:%x  line 478\n",d_start_shift[0]);
//printf("mi:%x line 479\n",mi);
     
   if(d_start_shift[SHIFT_LEFT] == mi)
       d_start_shift[SHIFT_LEFT] = d_start_shift[SHIFT_RIGHT];
     
     if(d_start_shift[SHIFT_LEFT]< mi) {
       shift_left_size = d_start_shift[SHIFT_RIGHT]  - d_start_shift[SHIFT_LEFT];
       start_shift_left =  d_start_shift[SHIFT_LEFT];
     } else
       shift_left_size = 0;	
     
     if (d_start_shift[SHIFT_RIGHT] < mi) {
       shift_right_size = mi - d_start_shift[SHIFT_RIGHT];
     } else 
       shift_right_size = 0;
...
CUDA_SAFE_CALL(cudaStreamAttachMemAsync(stream, gpu_kernel_input->d_zion.z0, 0, cudaMemAttachHost));
...
memcpy((sends[SHIFT_LEFT]+i*shift_left_size), gpu_kernel_input->d_zion.z0+i*d_mimax+start_shift_left, shift_left_size * sizeof(real));
...
memcpy((sends[SHIFT_RIGHT]+i*shift_right_size), gpu_kernel_input->d_zion.z0+i*d_mimax+start_shift_left+shift_left_size, shift_right_size * sizeof(real));
...
{
  //   printf("call_gpu_shifti_append_kernel call_gpu_shifti_append_kernel\n");
     if(mi_append==0)
       return;
     gpu_timer_start(gpu_kernel_input);
     cudaStream_t 	stream;
...
*/
//printf("d_start_shift[0]:%x  line 577\n",d_start_shift[0]);
     for(i=0;i<NPARAMS;i++)                           
//       CUDA_SAFE_CALL(cudaMemcpyAsync(gpu_kernel_input->d_zion.z0+i*d_mimax+d_start_shift[SHIFT_LEFT],particle_data+i*mimax_shift, mi_append* sizeof(real),cudaMemcpyHostToDevice,stream));
memcpy(gpu_kernel_input->d_zion.z0+i*d_mimax+d_start_shift[SHIFT_LEFT],particle_data+i*mimax_shift, mi_append* sizeof(real));


//printf("d_start_shift[0]:%x  line 581\n",d_start_shift[0]);
     CUDA_SAFE_CALL(cudaStreamSynchronize(stream));
     CUDA_SAFE_CALL(cudaStreamDestroy(stream));
...
//printf("d_start_shift[0]:%x  line 588\n",d_start_shift[0]);
     //free(transposed);
...
params.mi = gtc_input->global_params.mi;
//printf("Line 594****************************** params.mi:%x\n",params.mi);     
  
    gpu_kernel_input->gpu_timing.device_shift_time += gpu_timer_measure_end(gpu_kernel_input);
//     CUDA_SAFE_CALL(cudaFree(d_start_shift));
//printf("d_start_shift[0]:%x  line 595\n",d_start_shift[0]);
}
